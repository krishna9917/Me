...
class SearchMCXScreen extends ConsumerStatefulWidget {
  @override
  _SearchMCXScreenState createState() => _SearchMCXScreenState();
}

class _SearchMCXScreenState extends ConsumerState<SearchMCXScreen> {
    ...
    try {
      ...
      void processData(Datum datum) {
        ...
        if (matchingLiveData.instrumentIdentifier == datum.instrumentToken) {
          ...
          double previousBuyPrice = previousBuyPrices[datum.instrumentToken] ?? 0;
          double previousSellPrice = previousSellPrices[datum.instrumentToken] ?? 0;
          ...
          previousSellPrices[datum.instrumentToken.toString()] = currentSellPrice;

          final priceChange = matchingLiveData.priceChange ?? 0;
          final priceChangePercentage = matchingLiveData.priceChangePercentage ?? 0;

          final priceChangeColor = priceChange < 0 ? Colors.red : Colors.green;
          final priceChangePercentageColor = priceChangePercentage < 0 ? Colors.red : Colors.green;
  ...
  Future<List<GetLiveData>> fetchLiveData() async {
    try {
      ...
      var responseString = await ServicesApi().get_ApiwithHeader(
          ...
          "bXdSSmNIOHFOS2RoQTNES0hJYndwNmxjS1ZzbkVzZzVNSWdOVHZqRmh0MUFLNTJhSXZFYUdiQzVWS2Z3Z0V5"
              "TTNTejI1WWFoRm5MTWFJcTNlczJPajRlMk9qQ2dtZ3dYcExWVU1Laml4SlE9",ref);
  ...
  Future<void> fetchCategoryList() async {
    try {
      ...
      if (stdata != '' && stdata != null) {
        ...
        //
        // if (token != null && token.isNotEmpty) {
        String token = "bXdSSmNIOHFOS2RoQTNES0hJYndwNmxjS1ZzbkVzZzVNSWdOVHZqRmh0MUFLNTJhSXZFYUdiQzVWS2Z3Z0V5"
            "TTNTejI1WWFoRm5MTWFJcTNlczJPajRlMk9qQ2dtZ3dYcExWVU1Laml4SlE9";
        String url = 'https://www.onlinetradelearn.com/mcx/authController/getMcxCategoryList?userID=145';
  ...
  Future<void> RemoveToken(int categoryId, int isChecked) async {
    try {
      ...
      if (stdata != '' && stdata != null) {
        ...
        //
        // if (token != null && token.isNotEmpty) {
        String token = "bXdSSmNIOHFOS2RoQTNES0hJYndwNmxjS1ZzbkVzZzVNSWdOVHZqRmh0MUFLNTJhSXZFYUdiQzVWS2Z3Z0V5TTNTejI1WWFoRm5MTWFJcTNlczJPajRlMk9qQ2dtZ3dYcExWVU1Laml4SlE9";
        String url = 'https://www.onlinetradelearn.com/mcx/authController/addUserWatchList?userID=145&category_id=$categoryId&ischecked=$isChecked';

        print('Removing token with URL: $url');
                            ...
                            child: GestureDetector(
                              ...
                              child: Image.asset(
                                data['isChecked'] == 1
                                    ...
                                    : 'assets/unchecked.png', // Replace with the path to your unchecked image
                                width: 20.0, // Adjust the size of the image as needed
                                height: 20.0,
                              ),
                            ),
                          ),
